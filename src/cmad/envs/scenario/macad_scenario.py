...
These are directly ported from the scenarios defined in MACAD-GYM.
__author__ = PP
"""
...
def build_ma_scenario(
    map, actors, vehicles=0, pedestrians=0, max_steps=500, weathers=None
):
    if weathers is None:
        weathers = [0]
    scenario = {
...
class MacadScenario:
    """Multi Agent Scenarios for CARLA defined in MACAD-GYM"""
    ...
    SUI3C_TOWN3 = {
        "map": "Town03",
        "actors": {
            "car1": {"start": [70, -132.8, 8], "end": [127, -132, 8]},
            "car2": {"start": [84.3, -118, 9], "end": [120, -132, 8]},
            "car3": {
                "start": [43, -133, 4],
                "end": [100, -132, 8],
            },
    ...
    DEFAULT_SCENARIO_TOWN1 = build_ma_scenario(
        map="Town01",
        actors={"vehicle1": {"start": 128, "end": 133}},
        max_steps=2000,
        weathers=[0],
    )
    ...
    DEFAULT_SCENARIO_TOWN1_2 = build_ma_scenario(
        map="Town01",
        actors={"vehicle1": {"start": 133, "end": 65}},
        max_steps=2000,
        weathers=[0],
    )
    ...
    TOWN1_ALL = (
        TOWN1_STRAIGHT + TOWN1_ONE_CURVE + TOWN1_NAVIGATION + TOWN1_NAVIGATION_DYNAMIC
    )

    TOWN2_ALL = (
        TOWN2_STRAIGHT + TOWN2_ONE_CURVE + TOWN2_NAVIGATION + TOWN2_NAVIGATION_DYNAMIC
    )
    ...
    def resolve_scenarios_parameter(cls, scenarios_parameter):
        ...
        if isinstance(scenarios_parameter, dict):
            ...
            for scenario in scenarios_parameter:
                if isinstance(scenario, dict):
                    scenario_map.append(scenario)
                elif isinstance(scenario, str):
                    scenario_map.append(cls.get_scenario_parameter(scenario))
                else:
                    raise ValueError(f"Invalid scenario parameter: {scenario}")
