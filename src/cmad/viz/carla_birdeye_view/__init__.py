...
from cmad.viz.carla_birdeye_view import actors, cache
from cmad.viz.carla_birdeye_view.actors import SegregatedActors
from cmad.viz.carla_birdeye_view.colors import RGB
from cmad.viz.carla_birdeye_view.mask import (
    COLOR_ON,
    MAP_BOUNDARY_MARGIN,
    Coord,
    CroppingRect,
...
class BirdViewProducer:
    ...
    def __init__(
        ...
        self._world = Simulator.get_world()
        self._map = Simulator.get_map()
        ...
        if path.is_file():
            ...
            logger.info("Loaded static layers from cache file: %s", cache_path)
        else:
            logger.warning(
                "Cache file does not exist, generating cache at %s", cache_path
            )
            ...
            logger.info("Saved static layers to cache file: %s", cache_path)
