...
from cmad.misc.util import weak_lru
...
class Frame:
    ...
    ) -> Union[Transform, carla.Transform]:
    ...
    def get_action_by_id(
        self, id: int, use_carla_api: bool = False
    ) -> Union[Action, carla.VehicleControl, carla.WalkerControl]:
        ...
        for frame_info, frame in zip(frames[::2], frames[1::2]):
            ...
            for id in current_frame.transform:
                if current_frame.frame_id == 1:
                    ...
                    current_frame.velocity[id] = Vector3D(*(d_loc / time_diff))
