...
def get_tls(
    ...
    angle_threshold: float = math.pi / 4.0,
):
    ...
    for t in world.get_actors().filter("traffic.traffic_light"):
        ...
        if dist < distance_threshold:
            ...
            if (
                math.fabs(angle) > angle_threshold
                and math.fabs(angle) < math.pi - angle_threshold
            ):
                tls[t] = (dist, angle)
    if sort:
...
def change_tl_of_actor(
    actor: carla.Vehicle, traffic_light_state: carla.TrafficLightState
):
