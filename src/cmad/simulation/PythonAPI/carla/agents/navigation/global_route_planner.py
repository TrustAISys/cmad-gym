...
class GlobalRoutePlanner(object):
    ...
    def _localize(self, location: carla.Location) -> Tuple[int, int]:
        """
    ...
    def _turn_decision(
        self, index: int, route: list, threshold: float = math.radians(35)
    ) -> RoadOption:
        """
    ...
    def _find_closest_in_list(
        self, current_waypoint: carla.Waypoint, waypoint_list: List[carla.Waypoint]
    ) -> int:
    ...
    def _extra_trace_route_local(
        ...
        """
        route_trace = []
        # _path_search return: path as list of node ids (as int) of the graph self._graph
        route = self._path_search(origin, destination)
        current_waypoint = self._wmap.get_waypoint(origin)
        destination_waypoint = self._wmap.get_waypoint(destination)
