...
from cmad.srunner.scenarioconfigs.openscenario_configuration import (
    OpenScenarioConfiguration,
)
from cmad.srunner.scenariomanager.carla_data_provider import CarlaDataProvider
from cmad.srunner.scenariomanager.scenario_manager import ScenarioManager
from cmad.srunner.scenarios.open_scenario import OpenScenario
from cmad.srunner.scenarios.route_scenario import RouteScenario
from cmad.srunner.tools.route_parser import RouteParser
from cmad.srunner.tools.scenario_helper import teleport_actor
from cmad.srunner.tools.scenario_parser import ScenarioConfigurationParser
...
class SrunnerArgs:
    ...
    port: str = "2000"
    timeout: str = "30.0"
    trafficManagerPort: str = "8000"
    trafficManagerSeed: str = "0"
    sync: bool = False
    list: bool = False
    scenario: str = None
    openscenario: str = None
    openscenarioparams: str = None
    route: list = None
    agent: str = None
    agentConfig: str = ""
    output: bool = False
    ...
    outputDir: str = ""
    configFile: str = ""
    additionalScenario: str = ""
    debug: bool = False
    reloadWorld: bool = False
    record: str = ""
    randomize: bool = False
    repititions: int = 1
    waitForEgo: bool = False
...
class ScenarioRunner(object):
    ...
    def __init__(self, args: SrunnerArgs = None):
        """
    ...
    def _prepare_ego_vehicles(self, ego_vehicles):
        ...
        if not self._args.waitForEgo:
            for vehicle in ego_vehicles:
                pool_actor = CarlaDataProvider.get_hero_actor(vehicle.rolename)
                if pool_actor is not None:
                    teleport_actor(pool_actor, vehicle.transform)
                    self.ego_vehicles.append(pool_actor)
                else:
    ...
    def _load_and_wait_for_world(self, town, ego_vehicles=None):
        ...
        self._args.sync = CarlaDataProvider.is_sync_mode()
    ...
    def _load_scenario(self, config, clean_on_failed: bool = True):
        """
        Load the scenario given by config
        """
        ...
        if (
            ...
            self._args.trafficManagerPort = CarlaDataProvider.get_traffic_manager_port()
    ...
    def _run_scenario(self, config):
        """Run the scenario given by config"""
        try:
            ...
            self.manager.scenario_class.remove_all_actors()
            if self._args.record:
    ...
    def _load_openscenario(self, clean_on_failed: bool = True):
        """
        Parse/Load a scenario based on OpenSCENARIO, return the config
        """
        if not os.path.isabs(self._args.openscenario):
            self._args.openscenario = os.path.join(
                os.path.dirname(__file__),
                self._args.openscenario.replace("srunner/", ""),
            )
    ...
    def _run_openscenario(self):
        ...
        """
        # Load the scenario configurations provided in the config file
        config = self._load_openscenario()
        if not config:
            return False

        result = self._load_scenario(config)
        if not result:
            return False
