...
from cmad.srunner.scenariomanager.scenarioatomics.atomic_behaviors import Idle
from cmad.srunner.tools.scenario_helper import get_location_in_distance_from_wp
from cmad.srunner.scenarios.object_crash_vehicle import StationaryObjectCrossing
...
class ConstructionSetupCrossing(StationaryObjectCrossing):
    ...
    def create_cones_side(
        self, start_transform, forward_vector, z_inc=0, cone_length=0, cone_offset=0
    ):
        """
    ...
    def _create_construction_setup(self, start_transform, lane_width):
        ...
        for key, value in _initial_offset.items():
            ...
            transform = carla.Transform(
                start_transform.location, start_transform.rotation
            )
        ...
        side_transform.location -= (
            _initial_offset["cones"]["k"] * side_transform.rotation.get_forward_vector()
        )
        ...
        for i in range(len(_setup["lengths"])):
            ...
            side_transform.location += (
                side_transform.get_forward_vector()
                * _setup["lengths"][i]
                * _setup["offsets"][i]
            )
            side_transform.rotation.yaw += _perp_angle
