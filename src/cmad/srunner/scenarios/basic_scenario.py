...
import cmad.srunner.scenariomanager.scenarioatomics.atomic_trigger_conditions as conditions
...
from cmad.srunner.scenariomanager.weather_sim import WeatherBehavior
...
class BasicScenario(object):
    ...
    def __init__(
        ...
        self.scenario = Scenario(
            behavior_seq, criteria, self.name, self.timeout, self.terminate_on_failure
        )
    ...
    def _initialize_actors(self, config):
        ...
        if config.other_actors:
            for actor in config.other_actors:
                pool_actor = CarlaDataProvider.get_hero_actor(actor.rolename)
                if pool_actor is None:
                    ...
                    teleport_actor(pool_actor, actor.transform)
                    self.other_actors.append(pool_actor)
    ...
    def _setup_scenario_trigger(self, config):
        ...
        if config.trigger_points and config.trigger_points[0]:
            start_location = config.trigger_points[
                0
            ].location  # start location of the scenario
    ...
    def _setup_scenario_end(self, config):
        ...
        if ego_vehicle_route:
            if config.route_var_name is not None:
                set_name = "Reset Blackboard Variable: {} ".format(
                    config.route_var_name
                )
...
class Scenario(object):
    ...
    def _extract_nodes_from_tree(self, tree):  # pylint: disable=no-self-use
        ...
        if len(node_list) == 1 and isinstance(
            node_list[0], py_trees.composites.Parallel
        ):
            return []
