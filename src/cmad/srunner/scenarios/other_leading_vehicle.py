...
from cmad.srunner.tools.scenario_helper import get_waypoint_in_distance
...
class OtherLeadingVehicle(BasicScenario):
    ...
    def _create_behavior(self):
        ...
        keep_velocity.add_child(
            WaypointFollower(
                self.other_actors[0], self._first_vehicle_speed, avoid_collision=True
            )
        )
        keep_velocity.add_child(
            InTriggerDistanceToVehicle(self.other_actors[0], self.ego_vehicles[0], 55)
        )
        ...
        ego_drive_distance = DriveDistance(
            self.ego_vehicles[0], self._ego_vehicle_drive_distance
        )
        ...
        driving_in_same_direction.add_child(
            WaypointFollower(
                self.other_actors[1], self._second_vehicle_speed, avoid_collision=True
            )
        )

        sequence.add_child(
            ActorTransformSetter(self.other_actors[0], self._first_actor_transform)
        )
        sequence.add_child(
            ActorTransformSetter(self.other_actors[1], self._second_actor_transform)
        )
