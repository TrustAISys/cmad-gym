...
from cmad.srunner.scenarios.basic_scenario import BasicScenario
from cmad.srunner.tools.openscenario_parser import (
    OpenScenarioParser,
    oneshot_with_check,
    ParameterRef,
)
from cmad.srunner.tools.py_trees_port import Decorator
...
def repeatable_behavior(behaviour, name=None):
    ...
    if isinstance(behaviour, py_trees.composites.Sequence):
        ...
        sequence = py_trees.composites.Sequence(
            name="RepeatableBehaviour of {}".format(name)
        )
...
class StoryElementStatusToBlackboard(Decorator):
    ...
    def __init__(self, child, story_element_type, element_name):
        super(StoryElementStatusToBlackboard, self).__init__(
            name=child.name, child=child
        )
...
class OpenScenario(BasicScenario):
    ...
    def __init__(
        ...
        py_trees.blackboard.Blackboard().__dict__.clear()
    ...
    def _initialize_parameters(self):
        ...
        for i, global_action in enumerate(
            self.config.init.find("Actions").iter("GlobalAction")
        ):
            ...
            if global_action.find("ParameterAction") is not None:
                ...
                parameter_update = ChangeParameter(
                    parameter_ref,
                    value=ParameterRef(value),
                    rule=rule,
                    name=maneuver_name + "_%d" % i,
                )
                param_behavior.add_child(
                    oneshot_with_check(
                        variable_name="InitialParameters" + "_%d" % i,
    ...
    def _create_environment_behavior(self):
        ...
        weather_update = ChangeWeather(
            OpenScenarioParser.get_weather_from_env_action(
                self.config.init, self.config.catalogs
            )
        )
        ...
        env_behavior.add_child(
            oneshot_with_check(variable_name="InitialWeather", behaviour=weather_update)
        )
        env_behavior.add_child(
            oneshot_with_check(
                variable_name="InitRoadFriction", behaviour=road_friction
            )
        )
        ...
        for actor in self.config.other_actors + self.config.ego_vehicles:
            for carla_actor in self.other_actors + self.ego_vehicles:
                if (
                    ...
                    and "role_name" in carla_actor.attributes
                    and carla_actor.attributes["role_name"] == actor.rolename
                ):
                    actor_init_behavior = py_trees.composites.Sequence(
                        name="InitActor{}".format(actor.rolename)
                    )
                    ...
                    for private in self.config.init.iter("Private"):
                        if private.attrib.get("entityRef", None) == actor.rolename:
                            for private_action in private.iter("PrivateAction"):
                                for controller_action in private_action.iter(
                                    "ControllerAction"
                                ):
                                    ...
                                    controller_atomic = ChangeActorControl(
                                        ...
                                        args=args,
                                        scenario_file_path=os.path.dirname(
                    ...
                    if controller_atomic is None:
                        controller_atomic = ChangeActorControl(
                            carla_actor, control_py_module=None, args={}
                        )

                    actor_init_behavior.add_child(controller_atomic)

                    if actor.speed > 0:
                        actor_init_behavior.add_child(
                            ChangeActorTargetSpeed(
                                carla_actor, actor.speed, init_speed=True
                            )
                        )
    ...
    def _create_behavior(self):
        ...
        stories_behavior = py_trees.composites.Parallel(
            policy=py_trees.common.ParallelPolicy.SUCCESS_ON_ALL, name="OSCStories"
        )
        ...
        for story in self.config.stories:
            ...
            story_behavior = py_trees.composites.Parallel(
                policy=py_trees.common.ParallelPolicy.SUCCESS_ON_ALL, name=story_name
            )
                ...
                for sequence in act.iter("ManeuverGroup"):
                    ...
                    for _ in range(int(repetitions)):
                        ...
                        for (
                            ...
                            all_maneuvers
                        ):  # Iterates through both CatalogReferences and Maneuvers
                            ...
                            for event in maneuver.iter("Event"):
                                ...
                                for child in event.iter():
                                    if child.tag == "Action":
                                        for actor_id in actor_ids:
                                            ...
                                            maneuver_behavior = (
                                                StoryElementStatusToBlackboard(
                                                    maneuver_behavior,
                                                    "ACTION",
                                    ...
                                    if child.tag == "StartTrigger":
                                        ...
                                        parallel_condition_groups = (
                                            self._create_condition_container(
                                                child,
                                                story,
                                ...
                                maneuver_parallel.add_child(
                                    oneshot_with_check(
                                        variable_name=get_xml_path(story, sequence)
                                        + ">"
                                        + get_xml_path(
                                            maneuver, event
                                        ),  # See get_xml_path
                ...
                if list(start_triggers) is not None:
                    for start_condition in start_triggers:
                        parallel_start_criteria = self._create_condition_container(
                            start_condition, story, "StartConditions"
                        )
            ...
            stories_behavior.add_child(
                oneshot_with_check(
                    variable_name=get_xml_path(story, story)
                    + ">"
        ...
        if init_parameters is not None:
            behavior.add_child(
                oneshot_with_check(
                    variable_name="InitialParameterSettings", behaviour=init_parameters
                )
            )
        ...
        if env_behavior is not None:
            behavior.add_child(
                oneshot_with_check(
                    variable_name="InitialEnvironmentSettings", behaviour=env_behavior
                )
            )
        ...
        if init_behavior is not None:
            behavior.add_child(
                oneshot_with_check(
                    variable_name="InitialActorSettings", behaviour=init_behavior
                )
            )
            ...
            for condition in condition_group.iter("Condition"):
                ...
                if sequence is not None and maneuver is not None:
                    xml_path = (
                        ...
                        + ">"
                        + get_xml_path(maneuver, condition)
                    )  # See note in get_xml_path
                else:
                    xml_path = get_xml_path(story, condition)
                criterion = oneshot_with_check(
                    variable_name=xml_path, behaviour=criterion
                )
