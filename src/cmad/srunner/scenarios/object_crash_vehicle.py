...
from cmad.srunner.scenariomanager.timer import TimeOut
from cmad.srunner.scenarios.basic_scenario import BasicScenario
from cmad.srunner.tools.scenario_helper import get_location_in_distance_from_wp
...
class StationaryObjectCrossing(BasicScenario):
    ...
    def _create_behavior(self):
        ...
        lane_width = (
            ...
            .get_world()
            .get_map()
            .get_waypoint(self.ego_vehicles[0].get_location())
            .lane_width
        )
...
class DynamicObjectCrossing(BasicScenario):
    ...
    def __init__(
        ...
        self._adversary_type = (
            adversary_type  # flag to select either pedestrian (False) or cyclist (True)
        )
        self._walker_yaw = 0
    ...
    def _spawn_adversary(self, transform, orientation_yaw):
        ...
        if self._adversary_type is False:
            ...
            self._other_actor_target_velocity = (
                self._other_actor_target_velocity * self._num_lane_changes
            )
    ...
    def _spawn_blocker(self, transform, orientation_yaw):
        ...
        spawn_point_wp = (
            self.ego_vehicles[0].get_world().get_map().get_waypoint(transform.location)
        )

        self.transform2 = carla.Transform(
            carla.Location(
                x_static, y_static, spawn_point_wp.transform.location.z + 0.3
            ),
    ...
    def _create_behavior(self):
        ...
        if self._ego_route is not None:
            ...
            start_condition = InTimeToArrivalToVehicle(
                self.ego_vehicles[0], self.other_actors[0], self._time_to_reach
            )
        ...
        actor_drive = DriveDistance(
            self.other_actors[0], 0.5 * lane_width, name="walker drive distance"
        )
        ...
        actor_stop_crossed_lane = StopVehicle(
            self.other_actors[0], self._other_actor_max_brake, name="walker stop"
        )
        ego_pass_machine = DriveDistance(
            self.ego_vehicles[0], 5, name="ego vehicle passed prop"
        )
        ...
        scenario_sequence.add_child(
            ActorTransformSetter(
                self.other_actors[0], self.transform, name="TransformSetterTS3walker"
            )
        )
        scenario_sequence.add_child(
            ActorTransformSetter(
