...
class ResultOutputProvider(object):
    ...
    def __init__(
        self, data, result, stdout=True, filename=None, junitfile=None, jsonfile=None
    ):
        """
        ...
        self._start_time = time.strftime(
            "%Y-%m-%d %H:%M:%S", time.localtime(self._data.start_system_time)
        )
        self._end_time = time.strftime(
            "%Y-%m-%d %H:%M:%S", time.localtime(self._data.end_system_time)
        )
    ...
    def create_output_text(self):
        ...
        ratio = round(
            self._data.scenario_duration_game / self._data.scenario_duration_system, 3
        )
        ...
        list_statistics.extend(
            [["Ratio (System Time / Game Time)", "{}s".format(ratio)]]
        )
        ...
        for criterion in self._data.scenario.get_criteria():
            ...
            result = (
                "FAILURE"
                if criterion.test_status == "RUNNING"
                else criterion.test_status
            )
            actual_value = criterion.actual_value
            expected_value = criterion.expected_value_success

            list_statistics.extend(
                [[actor, criteria, result, actual_value, expected_value]]
            )
    ...
    def _write_to_junit(self):
        ...
        with open(self._junit, "w", encoding="utf-8") as junit_file:
            ...
            result_string = (
                '    <testcase name="Duration" status="run" time="{}" '
                'classname="Scenarios.{}">\n'.format(
