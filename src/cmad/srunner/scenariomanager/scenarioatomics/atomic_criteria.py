...
from cmad.srunner.scenariomanager.traffic_events import (
    TrafficEvent,
    TrafficEventType,
)
...
class DrivenDistanceTest(Criterion):
    ...
    def update(self):
        ...
        if self.actual_value > self.expected_value_success:
            ...
            self.expected_value_acceptable is not None
...
class AverageVelocityTest(Criterion):
    ...
    def __init__(
        ...
        """
        super(AverageVelocityTest, self).__init__(
            name, actor, avg_velocity_success, avg_velocity_acceptable, optional
        )
        self._last_location = None
        self._distance = 0.0
...
class CollisionTest(Criterion):
    ...
    def __init__(
        ...
        super(CollisionTest, self).__init__(
            name, actor, 0, None, optional, terminate_on_failure
        )
        self.logger.debug("%s.__init__()" % (self.__class__.__name__))
    ...
    def update(self):
        ...
        if (
            self.last_id
            and GameTime.get_time() - self.collision_time > self.MAX_ID_TIME
        ):
            self.last_id = None
    ...
    def _count_collisions(
        ...
        if self.other_actor_type:
            if self.other_actor_type == "miscellaneous":
                if (
                    "traffic" not in event.other_actor.type_id
        ...
        if (
            "static" in event.other_actor.type_id
            or "traffic" in event.other_actor.type_id
...
class ActorSpeedAboveThresholdTest(Criterion):
    ...
    def __init__(
        ...
        Class constructor.
        """
        super(ActorSpeedAboveThresholdTest, self).__init__(
    ...
    def update(self):
        ...
        if linear_speed is not None:
            if linear_speed < self._speed_threshold and self._time_last_valid_state:
                if (
                    GameTime.get_time() - self._time_last_valid_state
...
class OffRoadTest(Criterion):
    ...
    def __init__(
        ...
        """
        super(OffRoadTest, self).__init__(
            name, actor, 0, None, optional, terminate_on_failure
        )
        self.logger.debug("%s.__init__()" % (self.__class__.__name__))
...
class EndofRoadTest(Criterion):
    ...
    def __init__(
        ...
        Setup of the variables
        """
        super(EndofRoadTest, self).__init__(
            name, actor, 0, None, optional, terminate_on_failure
        )
        self.logger.debug("%s.__init__()" % (self.__class__.__name__))
...
class OnSidewalkTest(Criterion):
    ...
    def __init__(
        ...
        Construction with sensor setup
        """
        super(OnSidewalkTest, self).__init__(
            name, actor, 0, None, optional, terminate_on_failure
        )
        self.logger.debug("%s.__init__()" % (self.__class__.__name__))
    ...
    def update(self):
        ...
        elif current_wp.lane_type not in (
        ...
        distance_vector = (
            CarlaDataProvider.get_location(self._actor) - self._actor_location
        )
        distance = math.sqrt(
            math.pow(distance_vector.x, 2) + math.pow(distance_vector.y, 2)
        )
    ...
    def _set_event_dict(self, event, location, distance):
        ...
        """
        event.set_dict(
            {"x": location.x, "y": location.y, "z": location.z, "distance": distance}
        )
...
class OutsideRouteLanesTest(Criterion):
    ...
    ALLOWED_OUT_DISTANCE = (
        1.3  # At least 0.5, due to the mini-shoulder between lanes and sidewalks
    )
    MAX_ALLOWED_VEHICLE_ANGLE = 120.0  # Maximum angle between the yaw and waypoint lane
    MAX_ALLOWED_WAYPOINT_ANGLE = (
        150.0  # Maximum change between the yaw-lane angle between frames
    )
    WINDOWS_SIZE = (
        3  # Amount of additional waypoints checked (in case the first on fails)
    )
    ...
    def update(self):
        ...
        for index in range(
            ...
            wp_dir = (
                index_waypoint.transform.get_forward_vector()
    ...
    def _is_outside_driving_lanes(self, location):
        ...
        current_driving_wp = self._map.get_waypoint(
            location, lane_type=carla.LaneType.Driving, project_to_road=True
        )
        current_parking_wp = self._map.get_waypoint(
            location, lane_type=carla.LaneType.Parking, project_to_road=True
        )
        ...
        self._outside_lane_active = bool(
            distance > (lane_width / 2 + self.ALLOWED_OUT_DISTANCE)
        )
    ...
    def _is_at_wrong_lane(self, location):
        ...
        if current_waypoint.is_junction:
            ...
            if self._pre_ego_waypoint.is_junction:
                ...
                if (
                    vehicle_lane_angle < self.MAX_ALLOWED_VEHICLE_ANGLE
    ...
    def terminate(self, new_status):
        ...
        if self._wrong_distance > 0:
            ...
            outside_lane.set_dict(
                {"distance": self._wrong_distance, "percentage": percentage}
            )
        ...
        if (
            ...
            if self._previous_lane_waypoint.is_junction:
                ...
                vector_actor = np.array(
                    [
                        math.cos(
                            math.radians(self._actor.get_transform().rotation.yaw)
                        ),
                        math.sin(
                            math.radians(self._actor.get_transform().rotation.yaw)
                        ),
...
class InRadiusRegionTest(Criterion):
    ...
    def update(self):
        ...
        if self.test_status != "SUCCESS":
            in_radius = (
                math.sqrt(((location.x - self._x) ** 2) + ((location.y - self._y) ** 2))
                < self._radius
            )
            if in_radius:
...
class InRouteTest(Criterion):
    ...
    def __init__(
        ...
        """ """
        super(InRouteTest, self).__init__(
            name, actor, 0, terminate_on_failure=terminate_on_failure
        )
        self.logger.debug("%s.__init__()" % (self.__class__.__name__))
        self._actor = actor
            ...
            if self._current_index != closest_index:
                ...
                if not self._in_safe_route:
                    ...
                    out_route_percentage = (
                        100 * self._out_route_distance / self._accum_meters[-1]
                    )
...
class RouteCompletionTest(Criterion):
    ...
    def __init__(
        self, actor, route, name="RouteCompletionTest", terminate_on_failure=False
    ):
        """ """
        super(RouteCompletionTest, self).__init__(
            name, actor, 100, terminate_on_failure=terminate_on_failure
        )
        self.logger.debug("%s.__init__()" % (self.__class__.__name__))
        self._actor = actor
        self._route = route
    ...
    def update(self):
        ...
        if self._terminate_on_failure and (self.test_status == "FAILURE"):
            ...
            for index in range(
                ...
                dot_ve_wp = (
                    wp_veh.x * wp_dir.x + wp_veh.y * wp_dir.y + wp_veh.z * wp_dir.z
                )
                ...
                if dot_ve_wp > 0:
                    ...
                    self._percentage_route_completed = (
                        100.0
                        * float(self._accum_meters[self._current_index])
...
class RunningRedLightTest(Criterion):
    ...
    def __init__(self, actor, name="RunningRedLightTest", terminate_on_failure=False):
        ...
        Init
        """
        super(RunningRedLightTest, self).__init__(
            name, actor, 0, terminate_on_failure=terminate_on_failure
        )
        self.logger.debug("%s.__init__()" % (self.__class__.__name__))
        self._actor = actor
        ...
        for traffic_light, center, waypoints in self._list_traffic_lights:
            if self.debug:
                ...
                self._world.debug.draw_point(
                    center + carla.Location(z=z), size=0.2, color=color, life_time=0.01
                )
                for wp in waypoints:
            ...
            for wp in waypoints:
                ...
                ve_dir = CarlaDataProvider.get_transform(
                    self._actor
                ).get_forward_vector()
...
class RunningStopTest(Criterion):
    ...
    def __init__(self, actor, name="RunningStopTest", terminate_on_failure=False):
        """ """
        super(RunningStopTest, self).__init__(
            name, actor, 0, terminate_on_failure=terminate_on_failure
        )
        self.logger.debug("%s.__init__()" % (self.__class__.__name__))
        self._actor = actor
