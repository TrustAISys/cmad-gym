...
from cmad.srunner.tools.scenario_helper import get_distance_along_route
...
class InTriggerDistanceToVehicle(AtomicCondition):
    ...
    def update(self):
        ...
        distance = sr_tools.scenario_helper.get_distance_between_actors(
            self._actor,
...
class CheckParameter(AtomicCondition):
    ...
    def update(self):
        ...
        """
        new_status = py_trees.common.Status.RUNNING
        current_value = CarlaDataProvider.get_osc_global_param_value(
            self._parameter_ref
        )
