...
import carla
from cmad.simulation.PythonAPI.carla.agents.tools.misc import vector
from cmad.simulation.PythonAPI.carla.agents.navigation.local_planner import RoadOption

from cmad.srunner.scenariomanager.carla_data_provider import CarlaDataProvider


def teleport_actor(actor, transform):
    """
    Teleport an actor to a given transform, assure to be static
    """
    batch = []
    batch.append(carla.command.SetSimulatePhysics(actor.id, False))
    if isinstance(actor, carla.Vehicle):
        batch.append(
            carla.command.ApplyVehicleControl(
                actor.id, carla.VehicleControl(brake=1.0, hand_brake=True)
            )
        )
    else:
        batch.append(carla.command.ApplyWalkerControl(actor.id, carla.WalkerControl()))
    batch.append(carla.command.ApplyTransform(actor.id, transform))
    batch.append(carla.command.SetSimulatePhysics(actor.id, True))
    CarlaDataProvider._client.apply_batch_sync(
        batch, True if CarlaDataProvider.is_sync_mode() else False
    )
        ...
        if distance_squared < interval_length_squared:
            ...
            if prev_wp and curr_wp and wp:
                if wp.road_id in (prev_wp.road_id, curr_wp.road_id):
                    ...
                    if np.sign(wp.lane_id) == np.sign(prev_wp.lane_id) or np.sign(
                        wp.lane_id
                    ) == np.sign(curr_wp.lane_id):
...
def get_geometric_linear_intersection(ego_actor, other_actor):
    ...
    wp_other_1 = (
        CarlaDataProvider.get_world().get_map().get_waypoint(other_actor.get_location())
    )
...
def generate_target_waypoint_list_multilane(
    waypoint,
    change="left",  # pylint: disable=too-many-return-statements
...
def generate_target_waypoint_in_route(waypoint, route):
    ...
    while True:
        ...
        # Enter the junction
        if not reached_junction and (
            road_option in (RoadOption.LEFT, RoadOption.RIGHT, RoadOption.STRAIGHT)
        ):
            reached_junction = True

        # End condition for the behavior, at the end of the junction
        if reached_junction and (
            road_option not in (RoadOption.LEFT, RoadOption.RIGHT, RoadOption.STRAIGHT)
        ):
            break
...
def get_troad_from_transform(actor_transform):
    ...
    distance_from_rm_lane_edge = get_offset_transform(
