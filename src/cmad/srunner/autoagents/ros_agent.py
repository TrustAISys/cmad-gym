...
from cmad.srunner.autoagents.autonomous_agent import AutonomousAgent
...
class RosAgent(AutonomousAgent):
    ...
    def setup(self, path_to_conf_file):
        ...
        if not os.path.exists(start_script):
            raise IOError(
                "File '{}' defined by TEAM_CODE_ROOT invalid".format(start_script)
            )
        ...
        self.clock_publisher = rospy.Publisher(
            "clock", Clock, queue_size=10, latch=True
        )
        ...
        self.stack_process = subprocess.Popen(
            start_script, shell=True, preexec_fn=os.setpgrp
        )
